...
var process = require('OS/kernel/process');

class Consts {
}
// Population control.
Consts.maxNumberHarvester = 0;
Consts.maxNumberUpgrader = 1;
Consts.maxNumberBuilder = 2;
Consts.maxNumberRepairer = 1;
Consts.maxNumberCarrier = 2;
Consts.maxNumberPioneer = 3;
Consts.maxNumberPillager = 1;
Consts.maxNumberSoldier = 1;
Consts.maxNumberDiplomats = 1;
Consts.maxNumberCarrierTeleporter = 1;
Consts.maxNumberMeleeFightersForAnotherRoom = 3;
// Roles.
Consts.roleBuilder = 'builder';
Consts.roleCarrier = 'carrier';
Consts.roleCarrierTeleporter = 'carrierTeleporter';
Consts.roleHarvester = 'harvester';
Consts.roleMiner = 'miner';
Consts.roleMinerLinker = 'minerLinker';
Consts.roleRepairer = 'repairer';
Consts.roleUpgrader = 'upgrader';
Consts.rolePioneer = 'pioneer';
Consts.rolePillager = 'pillager';
Consts.roleSoldier = 'soldier';
Consts.rolefighterHealer = 'harvester';
Consts.roleFighterMelee = 'meleeFighter';
Consts.roleFighterMeleeForAnotherRoom = 'meleeFighterForAnotherRoom';
Consts.roleFighterRanged = 'rangedFighter';
Consts.roleDiplomat = 'diplomat';
// Room config.
Consts.roomLevelCanCreatePioneers = 4;
Consts.roomLevelCanReceivePioneers = 3;
// Creep repair.
Consts.minTicksBeforeRepairing = 50;
Consts.minTicksBeforeSpawningReplacement = 50;
Consts.shouldRenewCreeps = false;
// Misc.
Consts.garbageCollectionInterval = 100;
Consts.towerRepairSleepTimer = 100;
...
class HarvesterProcess extends process.Process {
    constructor() {
        super(...arguments);
        this._creep = null;
    }
    ...
    /** @param {Creep} creep **/
    run() {
        this._creep = Game.getObjectById(this.memory.creepId);
        if (!this._creep) {
        ...
        if (this._creep.memory.working) {
            ...
            if (!this._creep.memory.otherResources) {
                let otherResources = lodashExports.filter(RESOURCES_ALL, r => r !== RESOURCE_ENERGY && this._creep && this._creep.store.getUsedCapacity(r) > 0);
            ...
            if (target) {
                ...
                if (this._creep.memory.otherResources) {
                    this._creep.memory.otherResources.forEach(oR => {
                        ...
                        lodashExports.pull(this._creep.memory.otherResources, oR);
                    });
                }
...
class Kernel {
    ...
    addProcess(p, priority = 3 /* ProcessPriority.LowPriority */) {
        p.pid = this.getFreePid();
        p.status = 0 /* ProcessStatus.ALIVE */;
        p.priority = priority;
        this.processTable[p.pid] = p;
        if (priority === 1 /* ProcessPriority.Ticly */) {
            this.ticlyQueue.push(p);
        }
        if (priority === 2 /* ProcessPriority.TiclyLast */) {
            this.ticlyLastQueue.push(p);
        }
        if (priority === 3 /* ProcessPriority.LowPriority */) {
            this.lowPriorityQueue.push(p);
        }
        return p;
    }
    addProcessIfNotExists(p, priority = 3 /* ProcessPriority.LowPriority */) {
    ...
    killProcess(pid) {
        ...
        if (p) {
            p.status = -1 /* ProcessStatus.DEAD */;
        }
        return p;
    }
    forkProcess(origin, newProcess) {
    ...
    sleepProcessByTime(p, ticks) {
        return this.sleepProcess(p, { start: Game.time, duration: ticks });
    }
    sleepProcessByProcess(p, p2) {
        return this.sleepProcess(p, { pid: p2.pid });
    }
    sleepProcess(p, sleepInfo) {
        p.status = 1 /* ProcessStatus.SLEEP */;
        ...
        while (queue.length > 0) {
            ...
            while (process) {
                try {
                    ...
                    if (process.status === 0 /* ProcessStatus.ALIVE */) {
                        process.run();
                    }
                }
                catch (e) {
        ...
        for (let [pid, process] of Object.entries(storedTable)) {
            try {
                ...
                if (ProcessConstructor) {
                    let p = new ProcessConstructor(pid, process.parentPID, process.priority);
                    p.setMemory(memory);
                    this.processTable[p.pid] = p;
                    ...
                    if (process.priority === 1 /* ProcessPriority.Ticly */) {
                        this.ticlyQueue.push(p);
                    }
                    if (process.priority === 2 /* ProcessPriority.TiclyLast */) {
                        this.ticlyLastQueue.push(p);
                    }
                    if (process.priority === 3 /* ProcessPriority.LowPriority */) {
                        this.lowPriorityQueue.push(p);
                    }
...
const loop = () => {
    ...
    let p = kernel.addProcess(new HarvesterProcess(0, 0, 3 /* ProcessPriority.LowPriority */));
